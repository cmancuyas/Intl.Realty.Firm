...
namespace Intl.Realty.Firm.Controllers
{
    public class LeaseListingController : Controller
    {
        ...
        private readonly IExportService<LeaseListingExport> _exportService;
        private readonly IFileHandlerService _fileHandlerService;
        private readonly string _backupFileDirectory = "Uploads\\Documents\\";
        private readonly string _transactionTypeName = "Lease Listing";
        public LeaseListingController(IUnitOfWork unitOfWork,
                                    IConfigurationService configurationService,
                                    IExportService<LeaseListingExport> exportService,
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<LeaseListing> modelList = await _unitOfWork.LeaseListing.GetAllAsync(includeProperties: "IRFDeal,TransactionType,DealStatus") as List<LeaseListing> ?? throw new ArgumentException();

            List<LeaseListingViewModel> viewModelList = modelList.ToLeaseListingListViewModel();
        ...
        public async Task<IActionResult> ListPartialView()
        {
            ...
            var modelList = await _unitOfWork.LeaseListing
            ...
            LeaseListingListViewModel listViewModel = new LeaseListingListViewModel();

            listViewModel.LeaseListingsViewModel = modelList.ToLeaseListingListViewModel();
            listViewModel.Users = users.ToList();

            return PartialView("~/Views/LeaseListing/Partial/ListPartial.cshtml", listViewModel);
        }
        [HttpGet]
        public async Task<IActionResult> Create()
        {

            CreateLeaseListingViewModel viewModel = new CreateLeaseListingViewModel();
        ...
        public async Task<IActionResult> Create(CreateLeaseListingViewModel viewModel)
        {
            var userId = Session.UserId;
                ...
                if (files != null)
                {
                    foreach (var file in files)
                    {
                        ...
                        createFileUpload = new FileUpload()
                        {
                            ...
                            CreatedBy = userId,
                            LeaseListingId = leaseListingModelId,
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var leaseListingModel = await _unitOfWork.LeaseListing.GetAsync(x => x.Id == id, includeProperties: "IRFDeal,TransactionType,DealStatus");

            int leaseListingId = leaseListingModel.Id;

            var fileUploads = await _unitOfWork.FileUpload.GetAllAsync(x => x.LeaseListingId == leaseListingId, includeProperties: "LeaseListing,TransactionType,DocumentType");

            if (leaseListingModel == null)
            {
                return NotFound();
            }
            var viewModel = leaseListingModel.ToEditLeaseListingViewModel();
            ...
            viewModel.DocumentTypeList = await GetDocumentTypesFromDocumentTypeAssignment(leaseListingModel?.TransactionType?.Description ?? _transactionTypeName);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, EditLeaseListingViewModel viewModel)
        {
            if (id != viewModel.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                ...
                var leaseListingModel = await _unitOfWork.LeaseListing.GetAsync(x => x.Id == id, includeProperties: "IRFDeal");
                if (leaseListingModel == null)
                {
                    return NotFound();
                }
                var fileUploadsWithFiles = FilterFileUploadsWithFilesOnly(leaseListingModel.FileUploads);
                leaseListingModel.FileUploads = fileUploadsWithFiles;
                leaseListingModel.IRFDeal = viewModel.EditIRFDealViewModel?.ToIRFDealModel();

                string selectedDealStatusId = Request.Form["DealStatusDDL"].ToString();
                leaseListingModel.DealStatusId = Convert.ToInt32(selectedDealStatusId);
                // Update IRD Deal
                if (leaseListingModel.IRFDeal != null)
                {
                    await _unitOfWork.IRFDeal.UpdateAsync(leaseListingModel.IRFDeal);
                }

                leaseListingModel.IsActive = viewModel.IsActive;
                leaseListingModel.UpdatedBy = userId;
                leaseListingModel.UpdatedAt = DateTime.Now;

                // Update other properties as needed

                await _unitOfWork.LeaseListing.UpdateAsync(leaseListingModel);
        ...
        private string CreateFilePath(int leaseListingId)
        {
            var userId = Session.UserId;
            ...
            uploadPath = Path.Combine(uploadPath, userIdPath + leaseListingId + "\\");
        ...
        {
            var model = await _unitOfWork.LeaseListing.GetAsync(x => x.Id == id);
            if (model == null)
            {
                return NotFound();
            }

            var editLeaseListingViewModel = model.ToEditLeaseListingViewModel();

            return PartialView("~/Views/LeaseListing/Modal/DeleteModal.cshtml", editLeaseListingViewModel);
        }

        public async Task<IActionResult> DeleteMultipleModal(List<int> ids)
        {
            List<LeaseListing> modelList = new List<LeaseListing>();
            ...
            var transactionTypeViewModelList = modelList.ToLeaseListingListViewModel();

            IEnumerable<LeaseListingViewModel> modelIEnum = transactionTypeViewModelList.AsEnumerable();

            return PartialView("~/Views/LeaseListing/Modal/DeleteMultipleModal.cshtml", modelIEnum);
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            await _unitOfWork.LeaseListing.RemoveAsync(model);

            return RedirectToAction(nameof(Index));
        }
        public async Task<IActionResult> DeleteMultiple(IEnumerable<LeaseListingViewModel> viewModelList)
        {
            ...
            var leaseListingIds = viewModelList.Select(x => x.Id).ToList();
            var leaseListingListToBeDeleted = await _unitOfWork.LeaseListing.GetAllAsync(x => leaseListingIds.Contains(x.Id));

            var IRFDealIds = leaseListingListToBeDeleted.Select(x => x.IRFDealId).ToList();
            var IRFDealListToBeDeleted = await _unitOfWork.IRFDeal.GetAllAsync(x => IRFDealIds.Contains(x.Id));
            if (leaseListingIds.Any() && leaseListingListToBeDeleted.Any())
            {
                var fileUploadList = await _unitOfWork.FileUpload.GetAllAsync(x => leaseListingIds.Contains(x.LeaseListingId ?? 0));

                if (leaseListingListToBeDeleted != null)
                {
                    try
                    {
                        ...
                        await _unitOfWork.LeaseListing.RemoveRangeAsync(leaseListingListToBeDeleted);
        ...
        public async Task<IActionResult> DeleteFile(int id)
        {
            ...
            if (fileUpload != null)
            {
                ...
                return RedirectToAction(nameof(Edit), new { id = fileUpload.LeaseListingId });
            }
        ...
        public async Task<JsonResult> GetDocumentTypesThatHasFileUploads(int leaseListingId)
        {
            var fileUploads = await _unitOfWork.FileUpload.GetAllAsync(x => x.LeaseListingId == leaseListingId);
            var documentTypeIds = fileUploads.Select(x => x.DocumentTypeId).ToArray();

            return Json(documentTypeIds);
        }
        [HttpPost]
        public async Task<IActionResult> GetLeaseListingList([FromBody] DtParameters dtParameters)
        {
            ...
            var result = await _unitOfWork.LeaseListing.AsQueryableAsync(includeProperties: "IRFDeal,TransactionType,DealStatus");
            ...
            var totalResultsCount = await _unitOfWork.LeaseListing.CountAsync();

            // Prepare the result for DataTables
            var jsonResult = new DtResult<LeaseListing>
            {
        ...
        private IQueryable<LeaseListing> DefaultFilterSearch(IQueryable<LeaseListing> result, string searchBy)
        {
        ...
        private IQueryable<LeaseListing> FilterSearchWithParameters(IQueryable<LeaseListing> query, string searchBy)
        {
        ...
        public async Task<IActionResult> ExportTable([FromQuery] string format, [FromForm] string dtParametersJson)
        {
            ...
            if (dtParameters != default)
            {
                ...
                var result = _unitOfWork.LeaseListing.AsQueryable(includeProperties: "IRFDeal,TransactionType,DealStatus");
                ...
                var resultListForExport = resultList.ToLeaseListingListExport();
